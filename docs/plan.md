...
6. **Rastreo cortés y sin suplantación.** User-agent identificable (`MarIA-GEO-Audit/x.y (+https://maria.ar/bot)`) y headers de request completos y estándar (`Accept`, `Accept-Language` — negociación de contenido correcta, no evasión), máximo 1 request concurrente por dominio, respeto del `Crawl-delay`, sin autenticación, sin resolver challenges, sin rotar IP ni suplantar la firma TLS de un navegador. **No se suplanta el crawler de un tercero:** la política declarada para GPTBot, ClaudeBot, CCBot, PerplexityBot y Google-Extended se lee de `robots.txt` (dato público); la respuesta real del servidor se mide una sola vez, con el UA propio. Si un sitio rechaza a ese cliente identificado y con request completo, eso **es** el hallazgo, no un obstáculo a esquivar. *(Enmienda 10-09-2026 — `docs/decisiones/2026-09-10-headers-de-request.md`; ante diferencia manda `constitution.md`.)*
...
- **(b) Respuesta real:** código HTTP ante un request **completo e identificado** — user-agent propio `MarIA-GEO-Audit/x.y (+https://maria.ar/bot)` + headers estándar `Accept`/`Accept-Language` (los que manda cualquier navegador o `curl`; enmienda 10-09-2026). Un 403, un 503 o un fallo de obtención puntúa 0 y eleva el hallazgo a severidad **crítica**: es acceso bloqueado, no ausencia de estructura.
